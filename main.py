...
def main():
    ...
    while True:
        ...
        for playlist_name, playlist in playlists.items():
            ...
            if len(urls) > 0:
                logging.log(f"[#] Playlist \"{playlist_name}(id: {user_playlist_id})\" extends {len(urls)} playlists")
                ...
                for url in urls:
                    ...
                    if inherited_playlist_id == None:
                        logging.log("[!] Couldn't extract playlist id from \"{url}\"!")
                        continue
                ...
                if len(inherited_songs_to_add) > 0:
                    ...
                    inherited_songs_to_add, dups = utils.remove_duplicates_from_list(inherited_songs_to_add)  
                    dup_len = len(dups)
                    if dup_len > 0:
                        songs_to_add_len = len(inherited_songs_to_add)
                        logging.log(f"[#] Removed {dup_len} duplicates from the song list, {songs_to_add_len} out of {songs_to_add_len + dup_len} songs left to process")

                if exists(playlist_json_file_path):
                    # this playlist was already extended once
                    # so get user added songs and script added songs 

                    logging.log(f"[#] Playlist was already extended before, comparing data to file")
                    ...
                    if len(user_removed_songs) > 0:
                        ...
                        logging.log(", ".join([songs_cache.get(x, "UNKNOWN") for x in user_removed_songs]), False)
                    ...
                    if len(new_user_added_songs) > 0:
                        ...
                        logging.log(", ".join([songs_cache.get(x, "UNKNOWN") for x in new_user_added_songs]), False)
                    ...
                    if len(inherited_songs_to_remove) == 0 and len(inherited_songs_to_add) == 0:
                        ...
                        if len(inherited_songs_to_remove) > 0:
                            ...
                            logging.log(", ".join([songs_cache.get(x, "UNKNOWN") for x in inherited_songs_to_remove]))
                        ...
                        if len(inherited_songs_to_add) > 0:
                            ...
                            logging.log(", ".join([songs_cache.get(x, "UNKNOWN") for x in inherited_songs_to_add]), False)
                    ...
                    if len(script_added_songs_data) == 0:
                        ...
                        logging.log(f"[#] {len(script_added_songs_data)} songs need to be added to the playlist:")
                        logging.log(", ".join([songs_cache.get(x, "UNKNOWN") for x in script_added_songs_data]), False)
